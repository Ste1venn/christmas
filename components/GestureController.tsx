...
export const GestureController: React.FC<GestureControllerProps> = ({ onModeChange, currentMode, onHandPosition }) => {
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        try {
          ...
          if (!window.isSecureContext) {
            console.warn("Camera requires a secure context (HTTPS or localhost)");
            setGestureStatus("HTTPS Required");
            return;
          }
  ...
  return (
    <div className="absolute top-6 right-[8%] z-50 flex flex-col items-end pointer-events-none">
      ...
      {/* Status Overlay */}
      <div className="mt-2 bg-black/60 text-[#D4AF37] text-xs px-2 py-1 rounded font-mono border border-[#D4AF37]/30">
        Status: {gestureStatus}
      </div>

      {!isLoaded && gestureStatus !== "HTTPS Required" && (
        <div className="mt-1 text-white/50 text-[10px] text-center max-w-[18vw]">
          Allow camera access for magic
        </div>
      )}

      {gestureStatus === "HTTPS Required" && (
        <div className="mt-1 text-red-400 text-[10px] text-center max-w-[18vw]">
